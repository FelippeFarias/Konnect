...
│   │
│   ├── konnect-core/          # All tool logic (23 toolsets)
│   │   └── src/
│   │       ├── mcp/
...
The server does NOT expose all 238 tools (245 total with the 7 meta-tools) in `tools/list` by default — that would cost ~23K tokens of context on every listing. Instead:
...
- **23 toolsets, 238 tools** + 7 meta-tools (4 routing + 2 observability + 1 runtime diagnostic — see `tool-directory.md`)
...
- Full-catalog `tools/list` (all loaded): 245 tools (238 registered + 7 meta) / ~25K tokens
